...
```{r metrics_poly_last_ood}
# Do final fit - need to use recipe to get poly terms
wait_i_poly_mod_final_fit <- 
  wait_i_poly_mod %>% 
...
```{r poly_ood_predict_plot}
xy_q_out %>% 
  select(mean_wait_i) %>% 
  bind_cols(predict(wait_i_poly_mod_final_fit, new_data = xy_q_out)) %>% 
  ggplot() + geom_point(aes(x=mean_wait_i, y=.pred))
```
```{r metrics_poly_last_ood}
# Do final fit
wait_i_spline_mod_final_fit <- 
  wait_i_spline_mod %>% 
  fit(wait_i_poly_mod_formula, data = xy_q_in_train)
...
five parameters that outperformed poly and spline models with many more paramters.
...
I'll follow up with the results of fitting each of the three models on the "out of design"
data to see how they do when forced to extrapolate. However, there's more than enough
here for you guys to get going to integrate some of this into the paper.
