...
For the polynomial, random forest and spline models, I was able to use the `tidymodels` package
...
wait_i_poly_recipe_noq <- recipe(wait_i_poly_mod_formula_noq, data = xy_q_in_train) %>% 
  step_poly(patients_per_clinic_block, num_med_techs, num_rooms,
          vitals_time_mean, exam_time_mean, exam_time_cv2, post_exam_time_mean,
          degree = 2)
...
Here is poly that also includes the DM1 term.

```{r poly_wf_q}
# Create an empty model object of the appropriate type. Since poly regression
# is really just a linear regression model with some added features (powers of inputs) we can use the `lm` as the engine.
wait_i_poly_mod_q <- linear_reg(mode = "regression") %>% 
  set_engine(engine = "lm")

# Create a formula object. Later we'll use a recipe to generate the poly terms.
wait_i_poly_mod_q_formula <- mean_wait_i ~ patients_per_clinic_block + 
                         num_med_techs +
                         num_rooms + 
                         vitals_time_mean +
                         exam_time_mean +
...
wait_i_poly_recipe_q <- recipe(wait_i_poly_mod_q_formula, data = xy_q_in_train) %>% 
  step_poly(patients_per_clinic_block, num_med_techs, num_rooms,
          vitals_time_mean, exam_time_mean, exam_time_cv2, post_exam_time_mean,
          off_util_staff, off_util_room, off_util_physician, mean_wait_i_dm1,
          degree = 2)

# Now create a workflow object that uses the model and recipe above
wait_i_poly_wflow_q <- 
  workflow() %>% 
  add_model(wait_i_poly_mod_q) %>% 
  add_recipe(wait_i_poly_recipe_q)
...
```{r metrics_poly_kfold_q}
collect_metrics(wait_i_poly_res_q)
```

Now we'll refit one last model using the entire training set and assess its
performance on the in design test data.

```{r metrics_poly_last_q}
# Do last fit
wait_i_last_poly_res_q <- last_fit(wait_i_poly_wflow_q, xy_q_in_split)

collect_metrics(wait_i_last_poly_res_q)
```
The RMSE on test is slightly smaller than RMSE on train which is unusual but
can happen due to randomness.
...
```{r assess_wait_i_last_poly_res_q, fig.height=7, fig.width=7}
# Grab the predictions on the test data
assess_wait_i_last_poly_res_q <- collect_predictions(wait_i_last_poly_res_q)

# Plot actual vs predicted.
assess_wait_i_last_poly_res_q %>% 
  ggplot(aes(x = mean_wait_i, y = .pred)) + 
  geom_point(alpha = .15) +
  geom_abline(color = "red") + 
  coord_obs_pred() + 
  xlab("Actual mean initial wait time (min)") +
  ylab("Metamodel prediction") +
  ggtitle("Polynomial regression model (Q)")
```
...
wait_i_spline_recipe_noq <- recipe(wait_i_spline_mod_formula_noq, data = xy_q_in_train) %>% 
  step_ns(patients_per_clinic_block, num_med_techs, num_rooms,
          vitals_time_mean, exam_time_mean, exam_time_cv2, post_exam_time_mean,
          deg_free = 2)
...
And here's spline with the DM1 term included

```{r spline_wf_q}
# Create an empty model object of the appropriate type. Since spline regression
# is really just a linear regression model with some added features (the spline related stuff) we can use the `lm` as the engine.
wait_i_spline_mod_q <- linear_reg(mode = "regression") %>% 
  set_engine(engine = "lm")

# Create a formula object. Later we'll use a recipe to generate the spline terms.
wait_i_spline_mod_q_formula <- mean_wait_i ~ patients_per_clinic_block + 
                         num_med_techs +
                         num_rooms + 
                         vitals_time_mean +
                         exam_time_mean +
...
wait_i_spline_recipe_q <- recipe(wait_i_spline_mod_q_formula, data = xy_q_in_train) %>% 
  step_ns(patients_per_clinic_block, num_med_techs, num_rooms,
          vitals_time_mean, exam_time_mean, exam_time_cv2, post_exam_time_mean,
          off_util_staff, off_util_room, off_util_physician, mean_wait_i_dm1,
          deg_free = 2)
...
# Now create a workflow object that uses the model and recipe above
wait_i_spline_wflow_q <- 
  workflow() %>% 
  add_model(wait_i_spline_mod_q) %>% 
  add_recipe(wait_i_spline_recipe_q)
...
```{r metrics_spline_kfold_q}
collect_metrics(wait_i_spline_res_q)
```

Now we'll refit one last model using the entire training set and assess its
performance on the in design test data.

```{r metrics_spline_last_q}
# Do last fit
wait_i_last_spline_res_q <- last_fit(wait_i_spline_wflow_q, xy_q_in_split)

collect_metrics(wait_i_last_spline_res_q)
```


Plot the actual vs predicted on the in design test dataframe.

```{r assess_wait_i_last_spline_res_q, fig.height=7, fig.width=7}
# Grab the predictions on the test data
assess_wait_i_last_spline_res_q <- collect_predictions(wait_i_last_spline_res_q)

# Plot actual vs predicted.
assess_wait_i_last_spline_res_q %>% 
  ggplot(aes(x = mean_wait_i, y = .pred)) + 
  geom_point(alpha = .15) +
  geom_abline(color = "red") + 
  coord_obs_pred() + 
  xlab("Actual mean initial wait time (min)") +
  ylab("Metamodel prediction") +
  ggtitle("Natural spline regression model (Q)")
```
...
wait_i_rf_recipe_noq <- recipe(wait_i_rf_mod_formula_noq, data = xy_q_in_train)
...
Here is random forest with util terms and the D/M/1 term

```{r rf_wf_q}
# Create an empty model object of the appropriate type. 
wait_i_rf_mod_q <- rand_forest(mode = "regression") %>% 
  set_engine(engine = "ranger")

# Create a formula object. 
wait_i_rf_mod_q_formula <- mean_wait_i ~ patients_per_clinic_block + 
                         num_med_techs +
                         num_rooms + 
                         vitals_time_mean +
                         exam_time_mean +
...
# Create recipe that simply adds in the formula
wait_i_rf_recipe_q <- recipe(wait_i_rf_mod_q_formula, data = xy_q_in_train) 

# Now create a workflow object that uses the model and recipe above
wait_i_rf_wflow_q <- 
  workflow() %>% 
  add_model(wait_i_rf_mod_q) %>% 
  add_recipe(wait_i_rf_recipe_q)
...
```{r metrics_rf_kfold}
collect_metrics(wait_i_rf_res_q)
```
...
```{r metrics_rf_last_q}
# Do last fit
wait_i_last_rf_res_q <- last_fit(wait_i_rf_wflow_q, xy_q_in_split)

collect_metrics(wait_i_last_rf_res_q)
```
The RMSE on test is slightly smaller than RMSE on train which is unusual but
can happen due to randomness.
...
```{r assess_wait_i_last_rf_res_q, fig.height=7, fig.width=7}
# Grab the predictions on the test data
assess_wait_i_last_rf_res_q <- collect_predictions(wait_i_last_rf_res_q)

# Plot actual vs predicted.
assess_wait_i_last_rf_res_q %>% 
  ggplot(aes(x = mean_wait_i, y = .pred)) + 
  geom_point(alpha = .15) +
  geom_abline(color = "red") + 
  coord_obs_pred() + 
...
```{r wait_i_dm1}
ggplot(xy_q_in) + geom_point(aes(x=mean_wait_i, y=mean_wait_i_dm1), alpha = .15) + 
  geom_abline(color = "red") + 
  coord_obs_pred() + 
  xlab("Actual mean initial wait time (min)") +
  ylab("D/M/1 prediction") 

ggsave('plots/raw_dm1.png')
...
ggsave('plots/act_vs_pred_nls_dm1.png')
```
...
wait_i_poly_out_wflow_noq <- 
  workflow() %>% 
  add_model(wait_i_poly_mod) %>% 
  add_recipe(wait_i_poly_recipe_noq)

wait_i_poly_out_wflow_bq <- 
  workflow() %>% 
  add_model(wait_i_poly_mod) %>% 
  add_recipe(wait_i_poly_recipe)

wait_i_poly_out_wflow_q <- 
  workflow() %>% 
  add_model(wait_i_poly_mod) %>% 
  add_recipe(wait_i_poly_recipe_q)
  

wait_i_poly_out_fit_noq <- fit(wait_i_poly_out_wflow_noq, data = xy_q_in)
wait_i_poly_out_pred_noq <- predict(wait_i_poly_out_fit_noq, new_data = xy_q_out)

wait_i_poly_out_fit_bq <- fit(wait_i_poly_out_wflow_bq, data = xy_q_in)
wait_i_poly_out_pred_bq <- predict(wait_i_poly_out_fit_bq, new_data = xy_q_out)

wait_i_poly_out_fit_q <- fit(wait_i_poly_out_wflow_q, data = xy_q_in)
wait_i_poly_out_pred_q <- predict(wait_i_poly_out_fit_q, new_data = xy_q_out)
...
wait_i_poly_out_rmse_noq <- rmse_vec(xy_q_out$mean_wait_i, wait_i_poly_out_pred_noq$.pred)
wait_i_poly_out_rmse_bq <- rmse_vec(xy_q_out$mean_wait_i, wait_i_poly_out_pred_bq$.pred)
wait_i_poly_out_rmse_q <- rmse_vec(xy_q_out$mean_wait_i, wait_i_poly_out_pred_q$.pred)

sprintf("RMSE for poly model on out of design points NQ: %10.3f", wait_i_poly_out_rmse_noq)
sprintf("RMSE for poly model on out of design points BQ: %10.3f", wait_i_poly_out_rmse_bq)
sprintf("RMSE for poly model on out of design points AQ: %10.3f", wait_i_poly_out_rmse_q)
```

```{r metrics_spline_final_out, fig.height=7, fig.width=7}
wait_i_spline_out_wflow_noq <- 
  workflow() %>% 
  add_model(wait_i_spline_mod) %>% 
  add_recipe(wait_i_spline_recipe_noq)

wait_i_spline_out_wflow_bq <- 
  workflow() %>% 
  add_model(wait_i_spline_mod) %>% 
  add_recipe(wait_i_spline_recipe)

wait_i_spline_out_wflow_q <- 
  workflow() %>% 
  add_model(wait_i_spline_mod) %>% 
  add_recipe(wait_i_spline_recipe_q)
  

wait_i_spline_out_fit_noq <- fit(wait_i_spline_out_wflow_noq, data = xy_q_in)
wait_i_spline_out_pred_noq <- predict(wait_i_spline_out_fit_noq, new_data = xy_q_out)

wait_i_spline_out_fit_bq <- fit(wait_i_spline_out_wflow_bq, data = xy_q_in)
wait_i_spline_out_pred_bq <- predict(wait_i_spline_out_fit_bq, new_data = xy_q_out)

wait_i_spline_out_fit_q <- fit(wait_i_spline_out_wflow_q, data = xy_q_in)
wait_i_spline_out_pred_q <- predict(wait_i_spline_out_fit_q, new_data = xy_q_out)
...
wait_i_spline_out_rmse_noq <- rmse_vec(xy_q_out$mean_wait_i, wait_i_spline_out_pred_noq$.pred)
wait_i_spline_out_rmse_bq <- rmse_vec(xy_q_out$mean_wait_i, wait_i_spline_out_pred_bq$.pred)
wait_i_spline_out_rmse_q <- rmse_vec(xy_q_out$mean_wait_i, wait_i_spline_out_pred_q$.pred)

sprintf("RMSE for spline model on out of design points NQ: %10.3f", wait_i_spline_out_rmse_noq)
sprintf("RMSE for spline model on out of design points BQ: %10.3f", wait_i_spline_out_rmse_bq)
sprintf("RMSE for spline model on out of design points AQ: %10.3f", wait_i_spline_out_rmse_q)
```
...
wait_i_rf_out_wflow_noq <- 
  workflow() %>% 
  add_model(wait_i_rf_mod) %>% 
  add_recipe(wait_i_rf_recipe_noq)

wait_i_rf_out_wflow_bq <- 
  workflow() %>% 
  add_model(wait_i_rf_mod) %>% 
  add_recipe(wait_i_rf_recipe)

wait_i_rf_out_wflow_q <- 
  workflow() %>% 
  add_model(wait_i_rf_mod) %>% 
  add_recipe(wait_i_rf_recipe_q)

wait_i_rf_out_fit_noq <- fit(wait_i_rf_out_wflow_noq, data = xy_q_in)
wait_i_rf_out_pred_noq <- predict(wait_i_rf_out_fit_noq, new_data = xy_q_out)

wait_i_rf_out_fit_bq <- fit(wait_i_rf_out_wflow_bq, data = xy_q_in)
wait_i_rf_out_pred_bq <- predict(wait_i_rf_out_fit_bq, new_data = xy_q_out)

wait_i_rf_out_fit_q <- fit(wait_i_rf_out_wflow_q, data = xy_q_in)
wait_i_rf_out_pred_q <- predict(wait_i_rf_out_fit_q, new_data = xy_q_out)
...
wait_i_rf_out_rmse_noq <- rmse_vec(xy_q_out$mean_wait_i, wait_i_rf_out_pred_noq$.pred)
wait_i_rf_out_rmse_bq <- rmse_vec(xy_q_out$mean_wait_i, wait_i_rf_out_pred_bq$.pred)
wait_i_rf_out_rmse_q <- rmse_vec(xy_q_out$mean_wait_i, wait_i_rf_out_pred_q$.pred)

sprintf("RMSE for rf model on out of design points NQ: %10.3f", wait_i_rf_out_rmse_noq)
sprintf("RMSE for rf model on out of design points BQ: %10.3f", wait_i_rf_out_rmse_bq)
sprintf("RMSE for rf model on out of design points AQ: %10.3f", wait_i_rf_out_rmse_q)
```
...
going to be situational. It's unrealistic to expect much more.
* offered load and resource utilization based features are often simple to develop
and can be quite predictive. More sophisticated terms based on things like
wait times models require more domain knowledge but can be extremely predictive.
