...
in_splits <- vfold_cv(xy_q_in, v = kfold_number, repeats = kfold_repeats)

```

We can see that `in_splits` is a dataframe of `vfold_split` objects (which are just
...
first_resample <- in_splits$splits[[1]]
