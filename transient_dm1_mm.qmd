...
# Here's just being explicit - these are the default values so no really needed
wait_i_nls_mod <- nonlinear_reg() %>% set_engine("nls")
translate(wait_i_nls_mod, engine="nls")
```
...
wait_i_nls_mod_fit <- 
  wait_i_nls_mod %>% 
  fit(formula = wait_i_nls_mod_formula, data = xy_q_in_train, start = init_nls_wait_i)
