...
Unfortunately, the `caret` package does not support passing `method=nsl` to its
...
In a nutshell, the `rsample::vfold_cv` function just creates an [rsplit object](https://cran.r-project.org/web/packages/rsample/vignettes/rsample.html#individual-resamples-are-rsplit-objects) that contains the dataset partition information.

#### Initial patient wait time model
...
# Starting guess for the non-linear model fitting via nls()
init_nls_wait_i <- c(b1=.1, b2=-0.6, b3=2, b4=0.5, b5=2)

# Create formula object
waiti_nls_formula <- mean_wait_i ~ b1 * (num_med_techs ^ b2) * (mean_wait_i_dm1 ^ b3) * (staff_eff_svc_time_cv2 ^ b5) * (num_rooms ^ b4)

# Do the cross-validation fit

fit <- train(waiti_nls_formula, 
             data = xy_q_in, 
             method = method, 
