...
```{r rf_wf}
# Create an empty model object of the appropriate type. Since rf regression
# is really just a linear regression model with some added features (powers of inputs) we can use the `lm` as the engine.
wait_i_rf_mod <- rand_forest(mode = "regression") %>% 
  set_engine(engine = "ranger")

# Create a formula object. Later we'll use a recipe to generate the rf terms.
...
# Now we'll create recipe object that contains two steps - the formula and the 
# rfnomial transforms.
# For step_rf(), the default of degree = 2 corresponds to using up to squared terms.
# We'll allow rfnomial terms for all of our relevant inputs.
...
assess_wait_i_last_rf_res %>% 
  ...
  xlab("Actual mean initial wait time (min)") +
  ylab("Metamodel prediction") +
  ggtitle("rfnomial regression model")
```
...
```{r rf_noq}
# Create a formula object. Later we'll use a recipe to generate the rfnomial terms.
...
```{r assess_wait_i_last_rf_res_noq}
# Grab the predictions on the test data
...
assess_wait_i_last_rf_res_noq %>% 
  ...
  xlab("Actual mean initial wait time (min)") +
  ylab("Metamodel prediction") +
  ggtitle("rfnomial regression model with no utilization terms")
```
