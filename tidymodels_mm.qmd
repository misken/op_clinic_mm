...
the model fitter. Since `nls()` needs a starting guess for the parameter values,
...
Now we declare the main arguments for the model. This is intended for turning parameters I think. 
...
Now we have a custom model type named "nonlinear_reg" that we can use. Before
...
```{r}
predict(wait_i_nls_fit, new_data = xy_q_in_test) %>% 
  head()
```

But, we really need to add the k-crossfold resampling just as we did with the
polynomial and spline models. Before we try that, we can actually get a preview
of the problem by tweaking the previous example to use a simple tidy workflow
object. The model and formula objects are fine. Let's use them in a workflow.
