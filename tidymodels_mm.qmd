...
That study also was an opportunity to learn about the [caret]() package and do
...
I rewrote the discrete event simulation model using [SimPy]() and we have
...
work but did give me a chance to finally dig into the R tidymodels package. I
...
So, I rewrote the [outpatient clinic simulation model in Python](https://github.com/misken/op_clinic) (uses the SimPy library). The logic is the same as the original model but incorporates the changes above and automates a bunch of simulation output analysis to create metamodeling input data sets (similar to what I did for the obflow case study in the paper). Each of the 4860 scenarios was simulated with 50 independent replications which then formed the basis for computing the scenario specific summary stats that get used as metamodeling inputs. It takes a few hours to make all the simulation runs.
...
```{r assess_wait_i_last_poly_res_noq}
# Grab the predictions on the test data
...
Ok, that's the polynomial regression model. Let's do natural cubic spline next.
...
```{r assess_wait_i_last_spline_res_noq}
# Grab the predictions on the test data
