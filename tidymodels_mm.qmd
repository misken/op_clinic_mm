---
title: "Using tidymodels for simulation metamodeling (and a mini-rant on academic publishing)"
format: 
  html:
    self-contained: true
...
I've written a multipart blog series on using R to do simulation metamodeling. The
...
The following blog posts I have done over the past few years provide some background on building simulation metamodels for this patient flow network.
...
a paper that is currently under review (as of August 2022). For the new study,
...
Now I am revisiting this same scenario again for this current paper. After looking more closely at the original work, there were a few things I didn't like in terms of using it directly in the paper.
...
3. In the experimental design, there were five levels for the number of patients in each 4 hour clinic block. It looks like I had used levels 1, 3 and 5 as my training scenarios, and levels 2 and 4 for the test data. It would have been better if I had done a standard random train-test split with, say, 75% of the scenarios in train and the other 25% in split (there are 4860 scenarios in total). 

So, I rewrote the [outpatient clinic simulation model in Python](https://github.com/misken/op_clinic) using the SimPy library. The logic is the same as the original model but incorporates the changes above and automates a bunch of simulation output analysis to create metamodeling input data sets. Each of the 4860 scenarios was simulated with 50 independent replications which then formed the basis for computing the scenario specific summary stats that get used as metamodeling inputs. It takes a few hours to make all the simulation runs.
...
For this performance measure, I fit four different metamodels:
...
For the polynomial, spline and random forest models, I was able to use the `tidymodels` package
...
test data. The [Tidy Modeling with R](https://www.tmwr.org/) online book by Kuhn
...
For those who have used the sci-kit learn package in Python, tidymodels will feel
...
* "empty" model objects are created (parsnip)
* adding model formulas and various preprocessing steps are done by creating
a *recipe* (`recipes`)
* resampling schemes such as k-crossfold validation or bootstrapping (`rsample`)
* hyperparameter tuning (`tune` and `dial`)
* model fitting (`parsnip` and `tune`)
* model predictions (`parsnip`)
* model assessment (`yardstick` and `tune`)
* put it all together into a modeling workflow, similar to a pipeline in scikit-learn (`workflows`)
...
more in the next section. I'm using the `rsample::` function call just to highlight
the fact that these funtions are from the `rsample` package - we've loaded `tidymodel` so
there's really no need to do this.

```{r}
set.seed(23)
# Do the 75/25 split
xy_q_in_split <- rsample::initial_split(xy_q_in)
...
five parameters that outperformed poly and spline models with many more parameters in
both in and out of design predictions (interpolation and extrapolation)
* the nonlinear queueing based model makes sense in the context of the underlying process physics of the simulation model - each term was interpretable and had basis in theory,
